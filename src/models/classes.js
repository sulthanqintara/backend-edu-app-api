...
const getClasses = (query, hostname) =>
  new Promise((resolve, reject) => {
    ...
    const queryString = `SELECT * FROM classes WHERE name LIKE "%${keyword}%" AND category_id >= ? AND level LIKE "%${level}%" AND pricing >= ? LIMIT ? OFFSET ?`;
    db.query(
      queryString,
      [category_id, price, limit, offset],
      (error, result) => {
        if (error) return reject(error);
        if (!result.length) return reject(404);
        const queryCountTotal = `SELECT COUNT(id) AS total FROM classes WHERE name LIKE "%${keyword}%" AND category_id = ? AND level LIKE "%${level}%" AND pricing >= ?`;
        db.query(queryCountTotal, [category_id, price], (err, totalResult) => {
          if (err) return reject(err);
