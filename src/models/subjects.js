...
const getAverageScore = (query) => {
  return new Promise((resolve, reject) => {
    ...
    const queryString = `SELECT sc.id, sc.score, u.name AS student, sc.subject_id, su.name AS subject, su.subject_date AS "date", c.name AS "class_name" FROM scoring sc JOIN users u ON sc.user_id = u.id JOIN subjects su ON sc.subject_id = su.id JOIN classes c ON su.class_id = c.id WHERE su.class_id = ? AND sc.user_id = ?`;
